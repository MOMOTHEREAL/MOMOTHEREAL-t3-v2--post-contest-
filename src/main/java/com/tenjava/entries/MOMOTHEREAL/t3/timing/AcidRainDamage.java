    ...
    public void run() {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            if (player.getWorld().isThundering()) {
                if (isAtDayLight(player) && (!(player.getHealth() < 2d)) && player.getWorld().getDifficulty() != Difficulty.PEACEFUL && TenJava.isAcidRaining && TenJava.enabledFeatures && !TenJava.isInSnowBiome(player)) {
                    player.damage(1d);
